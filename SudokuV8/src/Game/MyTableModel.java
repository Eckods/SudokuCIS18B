...
public class MyTableModel extends AbstractTableModel{
     ...
     */
    public MyTableModel(Object rowValues[][], Object colValues[], String forbiddenCells[][], int pressed){
        for(int i = 0; i <9; i++){
            for(int j = 0; j < 9; j++){
    ...
    public void setValueAt(Object value, int row, int column) {
       ...
       else if (forbidden[row][column] == "Show"){
    ...
    public boolean isCellEditable(int row, int column) { 
        ...
        if (count > 0 )
            return false;
        else if (rowData[row][column] == forbidden[row][column] ){
            return false; 
        }else if (rowData[row][column] != ""){
            return false;
        }
       // else if(forbidden[row][column] == "Show"){
        //    return false;
