...
public class MainFrame extends JFrame{
    ...
    private void hintButtonActionPerformed(ActionEvent event){ 
        ...
        while(hintAvailable >0){
            ...
            if (model.rowData[rNumberRow][rNumberCol]==""){
                ...
                model.rowData[rNumberRow][rNumberCol] = sudokuCompletePuzzle[rNumberRow][rNumberCol];
                break;
            }
    ...
    private void solutionButtonActionPerformed(ActionEvent event){ 
        ...
        // Upon pressing show solution, increment the count to make all cells
        // un-editable
        count++;
        table.setModel(new MyTableModel(rowData, colData, sudokuForbidden, count));
        ...
        // Reset the count and disable buttons
        count = 0;
        disableButtons();
    }
    ...
    private void commitButtonActionPerformed(ActionEvent event){ 
        ...
        for(int i = 0; i < 9; i++){
            for(int j = 0; j < 9; j++){
                ...
                // If the rowData is not blank, it has a value, increment the count
                if (rowData[i][j] !=""){
                    count++;
                 }
                // If the model rowData is a correct value, increment count
                if (model.rowData[i][j].equals((Object)sudokuCompletePuzzle[i][j])){
                    count++;
                    System.out.println(" ->"+model.rowData[i][j] + "   " + (Object)sudokuCompletePuzzle[i][j]);
