...
public class MainFrame extends JFrame{
    ...
    private void hintButtonActionPerformed(ActionEvent event){   
        ...
        for(int i = 0; i < 9; i++){
            for(int j = 0; j < 9; j++){
                if(sudokuForbidden[i][j]=="EditableValueHere"){
                    hintAvailable ++;
                } 
            }
        }
        if (sudokuForbidden[rNumberRow][rNumberCol] == "EditableValueHere" && hintAvailable != 0){
            rowData[rNumberRow][rNumberCol] = sudokuCompletePuzzle[rNumberRow][rNumberCol];
            sudokuForbidden[rNumberRow][rNumberCol]= "" + sudokuCompletePuzzle[rNumberRow][rNumberCol];
            table.setModel(new MyTableModel(rowData, colData, sudokuForbidden));
        }
        else if(sudokuForbidden[rNumberRow][rNumberCol] != "EditableValueHere" && hintAvailable != 0) {
            hintButtonActionPerformed(event);
        }
        else{
