...
public class MainFrame extends JFrame{
    ...
    private final Object dupeData[][] = new Object[9][9];
    ...
    private void createGrid(){
        ...
        table = new JTable();
        model = new MyTableModel(rowData, colData, sudokuForbidden, count);
    ...
    private void newGameButtonActionPerformed(ActionEvent event){   
        generateGameBoard();
        table.setModel(new MyTableModel(rowData, colData, sudokuForbidden, count));
    ...
    private void hintButtonActionPerformed(ActionEvent event){   
        ...
        while(hintAvailable >0){
            ...
            if (model.rowData[rNumberRow][rNumberCol]==""){
                /*dupeData[rNumberRow][rNumberCol] = sudokuCompletePuzzle[rNumberRow][rNumberCol];
                sudokuForbidden[rNumberRow][rNumberCol]= "" + sudokuCompletePuzzle[rNumberRow][rNumberCol];
                table.setModel(new MyTableModel(dupeData, colData, sudokuForbidden));*/
                //sudokuForbidden[rNumberRow][rNumberCol] = "solved";
                System.out.println("trying to find random number" + hintAvailable);

                table.setValueAt(sudokuCompletePuzzle[rNumberRow][rNumberCol], rNumberRow, rNumberCol);
            ...
            if(hintAvailable ==0){
            otherButton[2].setDisabledIcon(new ImageIcon(getClass().getResource("/Images/"
                        + otherButtonImages[2] + "Hover.png")));
    ...
    private void solutionButtonActionPerformed(ActionEvent event){ 
        ...
        for(int i = 0; i < 9; i++){
            for(int j = 0; j < 9; j++){
                ...
                table.setValueAt(sudokuCompletePuzzle[i][j], i, j);
            }
        }
        count = 0;
    }
